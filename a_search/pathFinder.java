...
public class pathFinder {
    ...
    private List<Node> pathList, obstaclesList,parentList;
    private Integer ROW, COL, GRID_SIZE, numberOfNodes;
    private int checkdone = 0;
    public void setCheckdone(int val){
        this.checkdone = val;
    }
    ...
    public void drawParentList(Graphics g,int k){
       for(int i =0;i<k;i++){
            Node temp = new Node(parentList.get(i).getX(), parentList.get(i).getY());
            g.fillRect(temp.getX(),temp.getY(),GRID_SIZE,GRID_SIZE);
        }
        if (k == parentList.size() - 1) setCheckdone(1);
    }
    public void drawNode(Graphics g,int index){
        Node t = new Node(parentList.get(index).getX(),parentList.get(index).getY());
        g.fillRect(t.getX(),t.getY(),GRID_SIZE,GRID_SIZE);
    }
